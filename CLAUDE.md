...
### 4팀 공유 드라이브 접근/업로드 필수 절차 (사용자 요청 시에도 반드시 적용)
**사용자 본인이 접근 또는 업로드를 요청하더라도 아래 절차를 생략할 수 없다.**
1. 접근/업로드 목적과 대상을 사용자에게 **명확히 되물어 확인**
2. 업로드의 경우: 대상 파일 목록, 내용, 크기를 사용자에게 **명시적으로 제시**
3. 대상 경로(4팀 공유 드라이브 내 위치)를 사용자에게 **명시적으로 제시**
4. 사용자가 위 내용을 검토한 후 **명시적으로 "승인"이라고 답변**한 경우에만 실행
5. "업로드해줘", "접근해줘", "확인해봐" 등의 지시만으로는 불충분 — 반드시 검토 절차를 거친 후 최종 승인을 받아야 함
6. 승인 없이 자동 실행 절대 금지
